...
def simple_similarity(query, text):
    ...
    text_words = set(re.findall(r'[\u4e00-\u9fff]+|[a-zA-Z]+', text.lower()))
...
def retrieve_relevant_chunks(query, k=3):
    ...
    for i, chunk in enumerate(st.session_state.text_chunks):
        similarity = simple_similarity(query, chunk)
...
def generate_answer(query, context_docs):
    """使用简单的模板匹配生成答案，无需Ollama"""
    if not context_docs:
        return "文档中未找到相关答案", context_docs
    
    context = "\n\n".join([doc["content"] for doc in context_docs])
    
    # 简单的模板匹配答案生成
    answer = f"根据文档内容，以下是相关回答：\n\n"
    answer += f"【参考文档片段】\n"
    for i, doc in enumerate(context_docs[:2]):  # 最多使用2个最相关的片段
        answer += f"\n片段 {i+1} (来源: {doc['metadata']['source']}):\n"
        answer += f"{doc['content'][:300]}...\n" if len(doc['content']) > 300 else f"{doc['content']}\n"
    
    answer += f"\n\n基于以上参考文档，{query}"
    
    # 检查是否有明确的答案在文档中
    if "定义" in query or "是什么" in query:
        answer = f"关于{query.replace('是什么', '').replace('定义', '')}，"
        for doc in context_docs[:2]:
            answer += f"根据文档描述：{doc['content'][:200]}... "
    elif "如何" in query or "怎样" in query:
        answer = f"针对{query.replace('如何', '').replace('怎样', '')}，"
        for doc in context_docs[:2]:
            answer += f"文档中提到：{doc['content'][:200]}... "
    else:
        answer += f"\n\n请参考上述文档片段了解更多详情。"
...
if question := st.chat_input("请输入您的问题..."):
    ...
    with st.chat_message("assistant"):
        if not st.session_state.text_chunks:
            answer = "⚠️ 知识库尚未构建，请先上传文档并点击'构建知识库'"
            st.write(answer)
        else:
            try:
